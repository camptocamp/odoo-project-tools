...
def test_add_new(project):
    ...
    assert "odoo14-addon-edi_oca == 14.0.1.9.0" in REQUIREMENTS.read_text()
...
def test_add_new_with_version(project):
    ...
    assert f"odoo14-addon-edi_oca == {version}" in REQUIREMENTS.read_text()
    assert result.exit_code == 0
...
def test_upgrade(project):
    ...
    REQUIREMENTS.write_text("odoo14-addon-edi_oca == 14.0.1.8.0")
...
def test_add_new_with_pr(project):
    ...
    assert expected in REQUIREMENTS.read_text()
    assert result.exit_code == 0
...
def test_replace_pr(project):
    ...
    REQUIREMENTS.write_text(old_req)
    result = project.invoke(addon.add, [addon_name])
    version = "16.0.2.0.0"
    assert f"odoo-addon-edi_oca == {version}" in REQUIREMENTS.read_text()
    assert result.exit_code == 0
