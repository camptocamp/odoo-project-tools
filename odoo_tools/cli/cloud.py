...
def upload(dump_path, platform, customer, env, from_db):
    ...
    assert (dump_path or from_db) and not (
        dump_path and from_db
    ), "Either dump_path or from_db must be provided"
    if from_db:
...
@common_celebrimbor_options
@utils.click.handle_exceptions()
def restore_from_prod(platform, customer, env):
...
@utils.click.handle_exceptions()
def restore(dump_name, platform, customer, env):
