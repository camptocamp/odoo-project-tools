...
def bump(rel_type, new_version=None):
    ...
    repo = GitRepo(".")
    # Migration: Remove deprecated .bumpversion.cfg file
    # Custom configuration is allowed, but it is not recommended to do it in
    # .bumpversion.cfg anyways. bump-my-version recommends pyproject.toml instead
    bumpversion_cfg = build_path(".bumpversion.cfg")
    if bumpversion_cfg.is_file():
        click.echo("Removing deprecated .bumpversion.cfg file..")
        repo.index.remove([str(bumpversion_cfg)])
        bumpversion_cfg.unlink(missing_ok=True)
