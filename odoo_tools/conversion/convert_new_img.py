...
def remove_submodules(submodules):
    ...
    for section in submodules:
        ...
        run(["git", "rm", "-f", submodules[section].path], check=False)


def init_proj_v2():
    run(["rm", ".proj.cfg"], check=False)
    run(["otools-project", "init"], with_env=dict(PROJ_TMPL_VER="2"), check=False)
    run(
        [
            "git",
            "add",
...
def move_files():
    ...
    for dirname in odoo_dir.glob("local-src/*bundle"):
        ...
        run(["git", "rm", "-f", "-r", str(dirname)], check=False)
    ...
    for filename, destdir in to_move:
        ...
        run(["git", "mv", "-f", filename, destdir], check=False)
    ...
    if not patches_dir.is_dir():
        ...
        gitkeep.touch()
        run(["git", "add", str(gitkeep)], check=False)
...
def remove_files():
    ...
    for name in to_remove:
        ...
        if fpth.is_dir():
            # shutil.rmtree(fpth)
            run(["git", "rm", "-f", "-r", str(fpth)], check=False)
        elif fpth.is_file():
            # fpth.unlink()
            run(["git", "rm", "-f", str(fpth)], check=False)
        else:
...
def copy_dockerfile():
    ...
    run(["git", "rm", "-f", "odoo/Dockerfile"], check=False)
